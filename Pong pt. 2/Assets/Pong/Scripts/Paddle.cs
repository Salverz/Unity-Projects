...
public class Paddle : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision other)
    {
        ...
        // Play hit sound
        AudioSource paddleHit = GetComponent<AudioSource>();
        float pitch = (Mathf.Abs(currentVelocity.x) + 17) * 0.05f;
        paddleHit.pitch = pitch;
        paddleHit.Play();
    }

    public void setPaddleSize(float paddleSize)
    {
        GetComponent<Transform>().localScale = new Vector3(0.75f, 1f, paddleSize);
        minTravelHeight = (paddleSize - 10f) * .5f;
        maxTravelHeight = (10f - paddleSize) * .5f;
    }
